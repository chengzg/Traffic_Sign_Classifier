...
As mentioned earlier, the original training data set is not even distributed. So the first step of preprocessing the dataset is to augment the original dataset to make it move evenly distributed. Based on the original dataset, i use serveral steps to augment the data:
    1. rotate the original image by certain degree
    2. scale the original image by some factor
    3. Affine tranform the original image
    4. Manually introduce some noise on top of original dataset

To decided which category to augment, i use the current training set average number as the pivot point, if the current category is less than the average, then i will add the augmented image into the training set. The original image to be augmented is rnadomly choosen among the original dataset.
Refer to the image below for some more information:
...
**************************
Here need to test the effect of adding another layer
**************************
